...
if(isset($_POST['id'])){
	...
	$history = "SELECT *, p.property_id as p_id FROM history_owner_property h LEFT JOIN property p ON p.id = h.property_id WHERE h.id = '$id'";
	$qry = $conn->query($history) or trigger_error(mysqli_error($conn)." ".$history);
	$a = mysqli_fetch_assoc($qry); 

	 $data['name'] = ucwords($a['p_fname']." ".$a['p_mname']." ".$a['p_lname']);
	 $data['owner_address'] = ucwords($a['p_address']);
	 $data['p_id'] = $a['p_id'];
	 $currYear = date("Y");
	 $bithYear = date("Y",strtotime($a['p_bday']));
	 $age = $currYear - $bithYear;
	 $data['age'] = $age;
